...
**时间窗口对评分的影响 (V5.1攻击速率权重)**:

| 时间窗口 | 典型场景 | 攻击速率 | 评分影响 | 权重倍数 |
|---------|---------|---------|---------|---------|
| 30秒 | 实时检测 | 高速爆发 (1次/秒) | 显著提升评分 | 1.5-2.5x |
| 5分钟 | 标准评估 | 中等速率 (0.1次/秒) | 基准评分 | 1.0-1.5x |
| 15分钟 | 趋势分析 | 低速持续 (0.033次/秒) | 降低评分 | 0.7-1.0x |
| 1小时 | 长期监控 | 超低速 (0.008次/秒) | 大幅降低评分 | 0.5-0.7x |
...
**实际案例对比** (30次攻击, 2个IP, 2个端口):
```
30秒窗口  (1.0次/秒):   威胁评分 83.96 (CRITICAL) - 爆发性攻击
300秒窗口 (0.1次/秒):   威胁评分 79.56 (HIGH)     - 标准扫描
900秒窗口 (0.033次/秒): 威胁评分 76.83 (HIGH)     - 慢速扫描
3600秒窗口(0.008次/秒): 威胁评分 75.16 (HIGH)     - 超慢速扫描

评分差异: 30秒窗口比1小时窗口高 11.7% ✅
```
