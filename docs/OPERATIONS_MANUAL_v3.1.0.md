...
外部syslog日志
    |
    v
Logstash (10.174.1.229:32318, TCP)
    |
    v
...
系统接受syslog KV格式的攻击日志，通过TCP发送到 `10.174.1.229:32318` (Logstash端口)。
...
```bash
echo "log_type=1,customer_id=demo-customer,dev_serial=9d262111f2476d34,attack_mac=aa:bb:cc:dd:ee:10,attack_ip=192.168.1.50,response_ip=10.0.1.1,response_port=3389,log_time=$(date +%s)" | nc -q0 10.174.1.229 32318
```
...
for i in $(seq 1 50); do
  ...
  echo "log_type=1,customer_id=demo-customer,dev_serial=9d262111f2476d34,attack_mac=aa:bb:cc:dd:ee:${MAC_SUFFIX},attack_ip=192.168.1.${IP_LAST},response_ip=10.0.1.1,response_port=${PORT},log_time=${TS}" | nc -q0 10.174.1.229 32318
  sleep 0.1
done
echo "发送完毕，共50条"
```
...
- `Pending`: 资源不足或调度问题。
- `ImagePullBackOff`: 镜像拉取失败 (本系统使用本地镜像，不应出现)。
...
- 确认访问地址: `http://10.174.1.229:30080`
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v3.1.0 | 2026-04-02 | 首个全功能验证版本。修复端口攻击分布和威胁趋势图表数据链路。全部18个服务验证通过。ML检测6模型已加载。TIRE信誉引擎已验证。完成全链路数据流测试 (Syslog -> Logstash -> Kafka -> Flink -> Assessment -> DB -> Frontend)。 |

---

*本手册基于 v3.1.0 版本编写，适用于 K3s 单节点测试环境 (10.174.1.229)。*
*如有问题请联系系统管理员。*
