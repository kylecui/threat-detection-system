...
echo "等待4分钟以查看聚合结果..."
echo "可以使用以下命令查看最新告警："
echo "  docker exec postgres psql -U threat_user -d threat_detection -c \"SELECT id, attack_mac, tier, threat_score, attack_count, window_start, window_end FROM threat_alerts WHERE attack_mac = '$ATTACK_MAC' ORDER BY id DESC LIMIT 10;\""
