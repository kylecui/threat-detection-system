...
test_customer_management() {
    ...
    http_test "POST /customers" "POST" \
        "$CUSTOMER_MANAGEMENT_URL/api/v1/customers" \
    ...
    http_test "GET /customers/{id}" "GET" \
        "$CUSTOMER_MANAGEMENT_URL/api/v1/customers/$CUSTOMER_ID" \
        "" "200"
    ...
    http_test "POST /customers/{id}/devices" "POST" \
        "$CUSTOMER_MANAGEMENT_URL/api/v1/customers/$CUSTOMER_ID/devices" \
    ...
    http_test "GET /customers/{id}/devices/quota" "GET" \
        "$CUSTOMER_MANAGEMENT_URL/api/v1/customers/$CUSTOMER_ID/devices/quota" \
        "" "200"
    
    # 8. Configure notifications
    print_info "Test 8: Configure notifications"
    http_test "PUT /customers/{id}/notification-config" "PUT" \
        "$CUSTOMER_MANAGEMENT_URL/api/v1/customers/$CUSTOMER_ID/notification-config" \
...
test_alert_management() {
    ...
    ALERT_RESPONSE=$(curl -s -X POST "$ALERT_MANAGEMENT_URL/api/v1/alerts" \
    ...
    if [ ! -z "$ALERT_ID" ]; then
        ...
        http_test "GET /alerts/{id}" "GET" \
            "$ALERT_MANAGEMENT_URL/api/v1/alerts/$ALERT_ID" \
            "" "200"
        
        # 3. Resolve alert
        print_info "Test 3: Resolve alert"
        http_test "POST /alerts/{id}/resolve" "POST" \
            "$ALERT_MANAGEMENT_URL/api/v1/alerts/$ALERT_ID/resolve" \
    ...
    http_test "GET /alerts" "GET" \
        "$ALERT_MANAGEMENT_URL/api/v1/alerts?page=0&size=10" \
        "" "200"
...
test_data_ingestion() {
    ...
    http_test "POST /logs/ingest" "POST" \
        "$DATA_INGESTION_URL/api/v1/logs/ingest" \
    ...
    http_test "GET /logs/stats" "GET" \
        "$DATA_INGESTION_URL/api/v1/logs/stats" \
        "" "200"
...
test_threat_assessment() {
    ...
    http_test "POST /assessment/evaluate" "POST" \
        "$THREAT_ASSESSMENT_URL/api/v1/assessment/evaluate" \
    ...
    http_test "GET /assessment/trends" "GET" \
        "$THREAT_ASSESSMENT_URL/api/v1/assessment/trends?customerId=test-customer&days=7" \
        "" "200"
