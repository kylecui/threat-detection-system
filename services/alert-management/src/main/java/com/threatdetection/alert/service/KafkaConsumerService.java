...
public class KafkaConsumerService {
    ...
    public void consumeThreatEvents(
        ...
        // 收集需要发送通知的告警信息
        List<AlertNotificationInfo> notificationsToSend = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                try {
                    ...
                    if (result.isDedup) {
                        ...
                        alertsToSave.add(result.alert);
                        if (result.needsNotification) {
                            notificationsToSend.add(result.notificationInfo);
                        }
                    }
                    processedCount++;
            ...
            if (!alertsToSave.isEmpty()) {
                savedAlerts = alertService.saveAllAlerts(alertsToSave);
                log.info("批量保存完成: 保存了 {} 个告警到数据库", savedAlerts.size());
            }
            
            // 第三阶段: 异步发送通知（不阻塞主流程）
            if (!notificationsToSend.isEmpty() && !savedAlerts.isEmpty()) {
                sendNotificationsAsync(savedAlerts, notificationsToSend);
            }

            // 批量提交偏移量（所有消息处理完成后统一提交）
            acknowledgment.acknowledge();
            log.info("✅ 批量处理完成: 处理={}, 去重={}, 保存={}, 失败={}, 跳过={}, 总数={}", 
                    processedCount, deduplicatedCount, alertsToSave.size(), failureCount, skippedCount, batchSize);
    ...
    public void sendNotificationsAsync(List<Alert> savedAlerts, List<AlertNotificationInfo> notificationsToSend) {
        log.info("开始异步发送 {} 个通知", notificationsToSend.size());
        
        int successCount = 0;
        int failureCount = 0;
        
        // 为每个需要通知的告警发送邮件
        for (int i = 0; i < Math.min(notificationsToSend.size(), savedAlerts.size()); i++) {
            Alert savedAlert = savedAlerts.get(i);
            AlertNotificationInfo notifInfo = notificationsToSend.get(i);
        ...
        log.info("异步通知发送完成: 成功={}, 失败={}, 总数={}", 
                successCount, failureCount, notificationsToSend.size());
    }
